         ...
         // get the post labels from the entry
            ...
            if ("category" in entry) {
               for (var k = 0; k < entry.category.length; k++) {
                  pll += '<a href="javascript:filterPosts(\'' + entry.category[k].term + '\');" title="Click here to select all posts with the label \'' + entry.category[k].term + '\'">' + entry.category[k].term + '</a>,  ';
               }
...
// this function creates a three-column table and adds it to the screen
   ...
   if (numDisplayed == postTitle.length) {
      ...
      var tocNote = '<span class="toc-note">Show ' + numDisplayed + ' Post label \'';
      tocNote += postFilter + '\' trong tổng số '+ postTitle.length + ' Post<br/><br/></span>';
   }
